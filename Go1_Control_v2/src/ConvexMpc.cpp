...
void ConvexMpc::calculate_B_mat_c(double robot_mass, const Eigen::Matrix3d &go1_lumped_inertia, Eigen::Matrix3d root_rot_mat,
                                  Eigen::Matrix<double, 3, NUM_LEG> foot_pos) {
    // we need to calculate PLAN_HORIZON B matrices
    Eigen::Matrix3d go1_lumped_inertia_world;
    go1_lumped_inertia_world = root_rot_mat * go1_lumped_inertia * root_rot_mat.transpose();
    for (int i = 0; i < NUM_LEG; ++i) {
        B_mat_c.block<3, 3>(6, 3 * i) =
                go1_lumped_inertia_world.inverse() * Utils::skew(foot_pos.block<3, 1>(0, i));
...
void ConvexMpc::calculate_qp_mats(Go1ControlStates &state) {
