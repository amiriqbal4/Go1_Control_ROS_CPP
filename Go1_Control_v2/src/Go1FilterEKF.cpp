...
// Edited by Amir on 02/12/22.
//

#include "Go1FilterEKF.h"

Go1FilterEKF::Go1FilterEKF () {
    // constructor
    eye3.setIdentity();
    // C is fixed
    C.setZero();
...
Go1FilterEKF::Go1FilterEKF (bool assume_flat_ground_):Go1FilterEKF() {
    // constructor
...
void Go1FilterEKF::init_state(Go1ControlStates& state) {
...
void Go1FilterEKF::update_estimation(Go1ControlStates& state, double dt) {
    // update A B using latest dt
    A.block<3, 3>(0, 3) = dt * eye3;
    // B.block<3, 3>(0, 0) = 0.5*dt*dt * eye3; // check effect of this// Causes noisy estimates
    B.block<3, 3>(3, 0) = dt * eye3; //Check this again
...
//    r((i-1)*3+1:(i-1)*3+3) = body_v - R_er*leg_v;
    ...
    // final step
    // put estimated values back to Go1ControlStates& state
    for (int i = 0; i < NUM_LEG; ++i) {
