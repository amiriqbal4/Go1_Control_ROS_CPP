...
int main(int argc, char **argv) {
    ros::init(argc, argv, "hardware_GO1_qp_ctrl");
    ros::NodeHandle nh;
    ...
    std::unique_ptr<Go1HardwareROS> go1 = std::make_unique<Go1HardwareROS>(nh);
    ...
    std::thread main_thread([&]() {
        ...
        while (control_execute.load(std::memory_order_acquire) && ros::ok()) {
            ...
            dt_solver_time = ros::Time::now() - now;
            std::cout << "MainUpdateTime =  " << 1000*dt_solver_time.toSec()<< "ms" << std::endl;
